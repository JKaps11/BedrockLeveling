...
const PENIS_MOBS: Record<string, string> = {
...
const MOB_DISPLAY: Record<string, string> = {
  "minecraft:cow": "Cow",
  "minecraft:pig": "Pig",
...
const trackedSnowballs = new Set<string>();
...
export class PenisCollector {
  ...
  private subscribeMobDeaths(): void {
    world.afterEvents.entityDie.subscribe((event) => {
      ...
      try {
        ...
        (killer as Player).sendMessage(`\u00a7d\u00a77A \u00a7d${mobName} Penis \u00a77dropped!`);
      } catch {
  ...
  private subscribeSnowballHit(): void {
    world.afterEvents.projectileHitEntity.subscribe((event) => {
      if (event.projectile.typeId !== "minecraft:snowball") return;

      const snowballId = event.projectile.id;
      if (!trackedSnowballs.has(snowballId)) return;
      ...
      try {
        ...
        hitEntity.applyKnockback(
          { x: (dx / dist) * KNOCKBACK_STRENGTH, z: (dz / dist) * KNOCKBACK_STRENGTH },
          KNOCKBACK_VERTICAL,
        );
      } catch {
  ...
  private pollForCraftAnnouncement(): void {
    system.runInterval(() => {
      for (const player of world.getAllPlayers()) {
        if (announcedPlayers.has(player.id)) continue;
  ...
  static onBowUse(player: Player): void {
    // Cooldown check
    const now = system.currentTick;
    const lastUse = cooldowns.get(player.id) ?? 0;
    if (now - lastUse < COOLDOWN_TICKS) return;
