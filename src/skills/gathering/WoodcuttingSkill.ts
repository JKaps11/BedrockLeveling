import { Player, Block, BlockPermutation, ItemStack, system, Dimension, Vector3 } from "@minecraft/server";
...
export class WoodcuttingSkill extends BaseSkill {
  readonly skillType = SkillType.Woodcutting;
  readonly name = "Woodcutting";
  ...
  private treeFeller(player: Player, startBlock: Block, logType: string): void {
    const dimension = startBlock.dimension;
    const visited = new Set<string>();
    const toProcess: Vector3[] = [];

    // BFS to find all connected logs
    const queue: Vector3[] = [startBlock.location];
      ...
      for (let dx = -1; dx <= 1; dx++) {
        for (let dy = -1; dy <= 1; dy++) {
          for (let dz = -1; dz <= 1; dz++) {
            ...
            const np: Vector3 = { x: pos.x + dx, y: pos.y + dy, z: pos.z + dz };
        ...
        try {
          ...
          if (b && (LOG_BLOCKS.has(b.typeId) || LEAF_BLOCKS.has(b.typeId))) {
            ...
            if (isLog) {
              ...
              if (xpEntry) this.addXp(player, xpEntry.xp);
              // Spawn the log item
              try {
  ...
  private posKey(pos: Vector3): string {
