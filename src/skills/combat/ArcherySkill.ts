...
export class ArcherySkill extends BaseSkill {
  readonly skillType = SkillType.Archery;
  readonly name = "Archery";

  onArrowHit(player: Player, target: Entity, damage: number): void {
    ...
    if (target.typeId === "minecraft:player" && this.chanceCheck(player, DAZE_CHANCE_PER_LEVEL)) {
      try {
        (target as Player).addEffect("nausea", DAZE_DURATION_TICKS, { amplifier: 0 });
