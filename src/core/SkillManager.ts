import { world, system, Player, Entity, Block, EntityDamageCause, EquipmentSlot } from "@minecraft/server";
import { SkillType } from "../types/index.js";
import { SkillContext } from "../skills/BaseSkill.js";
import { PlayerDataManager } from "./PlayerDataManager.js";
import { AbilityManager } from "./AbilityManager.js";
import { AntiExploit } from "./AntiExploit.js";
...
import { getBlockXpEntry, MINING_BLOCKS, WOODCUTTING_BLOCKS, EXCAVATION_BLOCKS, HERBALISM_BLOCKS } from "../data/BlockXpValues.js";
...
export class SkillManager {
  ...
  private excavation: ExcavationSkill;
  private herbalism: HerbalismSkill;
  private fishing: FishingSkill;
  private swords: SwordsSkill;
  private axes: AxesSkill;
  private archery: ArcherySkill;
  private unarmed: UnarmedSkill;
  private taming: TamingSkill;
  private acrobatics: AcrobaticsSkill;
  private repair: RepairSkill;

  // Track sneaking state
  private sneakingPlayers: Set<string> = new Set();
  ...
  constructor(
    ...
    const ctx: SkillContext = { playerData, abilityManager, antiExploit, feedback };
  ...
  private subscribeBlockBreak(): void {
    world.afterEvents.playerBreakBlock.subscribe((event) => {
      const { player, brokenBlockPermutation, block } = event;
      if (this.antiExploit.isCreativeMode(player)) return;
  ...
  private subscribeCombat(): void {
    world.afterEvents.entityHurt.subscribe((event) => {
      ...
      if (cause === EntityDamageCause.fall && hurtEntity.typeId === "minecraft:player") {
        this.acrobatics.onFallDamage(hurtEntity as Player, damage);
        return;
      }
      ...
      if (attacker.typeId === "minecraft:player" && cause === EntityDamageCause.entityAttack) {
        ...
        if (SWORD_ITEMS.has(itemId)) {
          if (player.isSneaking) this.abilityManager.tryActivate(player, SkillType.Swords);
          this.swords.onHit(player, hurtEntity, damage);
        } else if (AXE_ITEMS.has(itemId)) {
          if (player.isSneaking) this.abilityManager.tryActivate(player, SkillType.Axes);
          this.axes.onHit(player, hurtEntity, damage);
        } else if (!mainhand || itemId === "") {
          if (player.isSneaking) this.abilityManager.tryActivate(player, SkillType.Unarmed);
  ...
  private subscribeSneaking(): void {
  ...
  private onStartSneak(player: Player): void {
    const equipment = player.getComponent("minecraft:equippable");
    const mainhand = equipment?.getEquipment(EquipmentSlot.Mainhand);
    const itemId = mainhand?.typeId ?? "";
