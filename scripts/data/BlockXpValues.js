...
export function getBlockXpEntry(blockTypeId) {
    return BLOCK_XP_VALUES[blockTypeId];
}
// Sets of block type IDs per skill for quick lookups
export const MINING_BLOCKS = new Set(Object.entries(BLOCK_XP_VALUES)
    .filter(([, e]) => e.skill === SkillType.Mining)
    .map(([k]) => k));
export const WOODCUTTING_BLOCKS = new Set(Object.entries(BLOCK_XP_VALUES)
    .filter(([, e]) => e.skill === SkillType.Woodcutting)
    .map(([k]) => k));
export const EXCAVATION_BLOCKS = new Set(Object.entries(BLOCK_XP_VALUES)
    .filter(([, e]) => e.skill === SkillType.Excavation)
    .map(([k]) => k));
export const HERBALISM_BLOCKS = new Set(Object.entries(BLOCK_XP_VALUES)
